...
async function initALWAYS() {
  ...
  await clearStalePageDataIfServerEmpty([ALWAYS_CONFIG.STORAGE_KEY, ALWAYS_CONFIG.PRODUCTLIST_STORAGE_KEY], ALWAYS_CONFIG.STORAGE_KEY);

  loadALWAYS();

  // === cloud-sync-init ===
  CloudSync.init(ALWAYS_CONFIG.STORAGE_KEY);
  await cloudPullAndRenderPage(ALWAYS_CONFIG, alwaysState || state, newALWAYSCard, renderALWAYS);
  startPageAutoSync(ALWAYS_CONFIG, alwaysState || state, newALWAYSCard, renderALWAYS, 10000);
...
function bindALWAYSPageLifecycle() {
  ...
  if (btnCloudSync) {
    btnCloudSync.addEventListener('click', () => manualCloudSyncPage(ALWAYS_CONFIG, alwaysState || state, newALWAYSCard, renderALWAYS));
  }
