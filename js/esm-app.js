...
async function initEsm() {
  ...
  await clearStalePageDataIfServerEmpty([ESM_CONFIG.STORAGE_KEY, ESM_CONFIG.PRODUCTLIST_STORAGE_KEY], ESM_CONFIG.STORAGE_KEY);

  loadEsm();

  // === cloud-sync-init ===
  CloudSync.init(ESM_CONFIG.STORAGE_KEY);
  await cloudPullAndRenderPage(ESM_CONFIG, esmState || state, newEsmCard, renderEsm);
  startPageAutoSync(ESM_CONFIG, esmState || state, newEsmCard, renderEsm, 10000);
...
function bindEsmPageLifecycle() {
  ...
  if (btnCloudSync) {
    btnCloudSync.addEventListener('click', () => manualCloudSyncPage(ESM_CONFIG, esmState || state, newEsmCard, renderEsm));
  }
