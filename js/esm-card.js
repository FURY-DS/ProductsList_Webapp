/* =====================================================
   esm-card.js - ESM 카드 렌더링 및 보드 관리
   ===================================================== */

let esmBoardEl = null;

/** 보드 요소 초기화 */
function initEsmBoard() {
  esmBoardEl = document.getElementById('board');
  document.documentElement.style.setProperty('--columns', ESM_CONFIG.COLUMNS);
}
...
/** ESM 카드용 이미지 결정 (직접 업로드한 사진 우선, 없으면 상품리스트 조회) */
function getEsmImage(card) {
...
/** 모든 카드 한 번에 접기/펼치기 */
function toggleAllEsmCards(collapse) {
  if (esmState.cards.length === 0) return;
  esmState.cards.forEach(c => c.isCollapsed = collapse);
  const result = saveEsm();
  renderEsm();
  reportSaveResult(result, ESM_CONFIG.MESSAGES, collapse ? ESM_CONFIG.MESSAGES.ALL_COLLAPSED : ESM_CONFIG.MESSAGES.ALL_EXPANDED);
}

/** 전체 렌더링 */
function renderEsm() {
  esmBoardEl.innerHTML = '';
  updateEsmSearchCount();
  updateEsmToggleAllButton();

  if (esmState.cards.length === 0) {
    renderEsmEmptyState(ESM_CONFIG.MESSAGES.EMPTY_TITLE, ESM_CONFIG.MESSAGES.EMPTY_DESC, true);
    return;
  }

  const q = esmSearchQuery.trim();
  const filtered = esmState.cards
    .map((card, originalIdx) => ({ card, originalIdx }))
    .filter(({ card }) => matchesEsmQuery(card, q));

  if (filtered.length === 0) {
    renderEsmEmptyState(ESM_CONFIG.MESSAGES.NO_RESULT_TITLE, ESM_CONFIG.MESSAGES.NO_RESULT_DESC(escapeAttr(q)), false);
    return;
  }

  const cols = [];
  for (let i = 0; i < ESM_CONFIG.COLUMNS; i++) {
    cols.push(document.createElement('div'));
  }
  filtered.forEach((entry, idx) => {
    cols[idx % ESM_CONFIG.COLUMNS].appendChild(renderEsmCard(entry.card, entry.originalIdx));
  });
  cols.forEach(c => esmBoardEl.appendChild(c));
}

/** 빈 상태 렌더링 */
function renderEsmEmptyState(title, desc, showAddBtn) {
  const empty = document.createElement('div');
  empty.className = 'empty';
  empty.style.gridColumn = '1 / -1';
  let html = `<h2>${title}</h2><p>${desc}</p>`;
  if (showAddBtn) {
    html += `<button class="btn btn-add" id="empty-add">${ESM_CONFIG.MESSAGES.EMPTY_ADD_BTN}</button>`;
  }
  empty.innerHTML = html;
  esmBoardEl.appendChild(empty);
  if (showAddBtn) {
    document.getElementById('empty-add').addEventListener('click', addEsmCard);
  }
}

/** 개별 카드 렌더링 */
function renderEsmCard(card, idx) {
  ...
  const actionBtn = card.isEditing
    ? `<button class="btn btn-add" data-action="save" title="입력한 내용 저장">${ESM_CONFIG.MESSAGES.BTN_SAVE}</button>`
    : `<button class="btn btn-cancel" data-action="edit" title="다시 수정하기">${ESM_CONFIG.MESSAGES.BTN_EDIT}</button>`;

  const toggleBtn = card.isCollapsed
    ? `<button class="btn btn-toggle" data-action="expand" title="상세 정보 펼치기">${ESM_CONFIG.MESSAGES.BTN_EXPAND}</button>`
    : `<button class="btn btn-toggle" data-action="collapse" title="상세 정보 숨기기">${ESM_CONFIG.MESSAGES.BTN_COLLAPSE}</button>`;

  const sellerCodeHtml = !card.isBundle && card.isEditing
    ? `<div class="field seller-code-field"><label>${ESM_CONFIG.FIELDS.sellerCode.label}</label><input type="text" name="sellerCode" value="${escapeAttr(card.sellerCode)}" placeholder="${escapeAttr(ESM_CONFIG.FIELDS.sellerCode.placeholder)}" /></div>`
    : (!card.isBundle && !card.isEditing
        ? `<span class="card-badge" title="${escapeAttr(ESM_CONFIG.FIELDS.sellerCode.label)}">${escapeAttr(card.sellerCode || '')}</span>`
        : `<span class="card-badge bundle-badge">${escapeAttr(ESM_CONFIG.MESSAGES.BUNDLE)}</span>`);
  ...
  // ----- 본문 -----
  if (card.isCollapsed) {
    wrap.appendChild(renderEsmPhotoRow(card));
  } else {
    wrap.appendChild(renderEsmPhotoRow(card));
    wrap.appendChild(renderEsmCalcRows(card));
    if (card.isBundle) {
    ...
    if (!card.isBundle && card.isEditing) {
      ...
      bundleAdd.innerHTML = `<button class="btn btn-toggle" data-action="add-bundle-mode">${ESM_CONFIG.MESSAGES.BTN_ADD_BUNDLE}</button>`;
    ...
    footer.innerHTML = `
      <button class="btn btn-add" data-action="add">${ESM_CONFIG.MESSAGES.BTN_ADD}</button>
      <button class="btn btn-remove" data-action="delete">${ESM_CONFIG.MESSAGES.BTN_DELETE}</button>
    `;
    wrap.appendChild(footer);
  }

  bindEsmCardEvents(wrap, card);
  return wrap;
}

/** 사진 + 상품명/옵션명 row */
function renderEsmPhotoRow(card) {
  const row = document.createElement('div');
  row.className = 'card-row';

  const imgSrc = getEsmImage(card);
  ...
  fields.appendChild(makeEsmField('name', card.name, !card.isEditing));
  fields.appendChild(makeEsmField('option', card.option, !card.isEditing));
...
function renderEsmCalcRows(card) {
  ...
  row1.appendChild(makeEsmField('finalCost', card.finalCost, true));
  row1.appendChild(makeEsmField('sellingPrice', card.sellingPrice, !card.isEditing));
  row1.appendChild(makeEsmFeeField(card));
  container.appendChild(row1);

  // 2행: 창고택배비 | 마켓택배비 | 최종이익(반) | 2개이상구매(반)
  const row2 = document.createElement('div');
  row2.className = 'field-row split-profit';
  row2.appendChild(makeEsmField('warehouseFee', card.warehouseFee, !card.isEditing));
  row2.appendChild(makeEsmField('marketFee', card.marketFee, !card.isEditing));
  row2.appendChild(makeEsmField('finalProfit', card.finalProfit, true));
  row2.appendChild(makeEsmField('multiBuyProfit', card.multiBuyProfit, true));
...
function makeEsmFeeField(card) {
  const f = document.createElement('div');
  f.className = 'field fee-field';
  const def = ESM_CONFIG.FIELDS.feeRate;
  ...
  f.innerHTML = `
    ...
    <input type="text" name="feeAmount" class="highlight" readonly value="${escapeAttr(formatNumber(parseNum(card.feeAmount)))}" placeholder="${escapeAttr(ESM_CONFIG.FIELDS.feeAmount.placeholder)}" />
  `;
  return f;
}

/** 단일 입력 필드 생성 */
function makeEsmField(name, value, readonly) {
  const def = ESM_CONFIG.FIELDS[name];
...
function renderBundleSection(card) {
  ...
  title.textContent = ESM_CONFIG.MESSAGES.BUNDLE;
  ...
  if (card.isEditing) {
    ...
    addBtn.textContent = ESM_CONFIG.MESSAGES.BTN_ADD_ITEM + ' ' + ESM_CONFIG.MESSAGES.BUNDLE;
...
function renderBundleItem(card, item, idx) {
  ...
  const roAttr = card.isEditing ? '' : 'readonly';
  codeField.innerHTML = `
    <label>${ESM_CONFIG.FIELDS.sellerCode.label}</label>
    <input type="text" name="itemSellerCode" value="${escapeAttr(item.sellerCode)}" placeholder="${escapeAttr(ESM_CONFIG.FIELDS.sellerCode.placeholder)}" ${roAttr} />
  `;
  ...
  totalField.innerHTML = `
    <label>${ESM_CONFIG.FIELDS.itemTotal.label}</label>
    <input type="text" name="itemTotal" class="highlight" readonly value="${escapeAttr(formatNumber(parseNum(item.total)))}" placeholder="${escapeAttr(ESM_CONFIG.FIELDS.itemTotal.placeholder)}" />
  `;
  ...
  if (card.isEditing) {
    ...
    removeBtn.textContent = ESM_CONFIG.MESSAGES.BTN_REMOVE_ITEM;
...
/** 카드 내 이벤트 바인딩 */
function bindEsmCardEvents(wrap, card) {
  // 입력 변경 (실시간 계산용)
  wrap.addEventListener('input', (e) => {
    const t = e.target;
    if (!t.name) return;
    const c = findEsmCard(card.id);
    if (!c) return;
  ...
  wrap.addEventListener('click', (e) => {
    ...
    if (photoBox || bundleThumb) {
      ...
      if (photoBox) {
        triggerEsmPhotoUpload(c.id, 'card');
      } else {
        const itemId = bundleThumb.closest('.bundle-item')?.dataset.itemId;
        if (itemId) triggerEsmPhotoUpload(c.id, 'bundle', itemId);
      }
      return;
    }
    ...
    if (action === 'add') {
      addEsmCardAfter(card.id);
    } else if (action === 'delete') {
      confirmDeleteEsm(card.id);
    } else if (action === 'save') {
      saveEsmCard(card.id);
    } else if (action === 'edit') {
      editEsmCard(card.id);
    } else if (action === 'collapse' || action === 'expand') {
      toggleEsmCollapse(card.id);
...
function updateSingleProductFromCode(card, code) {
  ...
  const product = lookupProductlistByCode(code);
  if (!product) {
    showToast(ESM_CONFIG.MESSAGES.PRODUCT_NOT_FOUND(code));
    return;
  }
...
/** 카드 저장 */
function saveEsmCard(cardId) {
  const c = findEsmCard(cardId);
  if (!c) return;
  syncEsmCardFromDOM(cardId);
  c.isEditing = false;
  const result = saveEsm();
  renderEsm();
  reportSaveResult(result, ESM_CONFIG.MESSAGES, ESM_CONFIG.MESSAGES.SAVED);
}

/** 렌더링된 DOM의 현재 입력값을 상태 객체에 동기화 (저장 직전 안전 장치) */
function syncEsmCardFromDOM(cardId) {
...
/** 카드 접기/펼치기 */
function toggleEsmCollapse(cardId) {
  const c = findEsmCard(cardId);
  if (!c) return;
...
function triggerEsmPhotoUpload(cardId, type, itemId) {
  ...
  input.addEventListener('change', async (e) => {
    ...
    if (file.size > ESM_CONFIG.IMAGE_MAX_SIZE_BYTES) {
    ...
    try {
      ...
      const result = saveEsm();
      renderEsm();
      reportSaveResult(result, ESM_CONFIG.MESSAGES, '사진을 변경했어요');
    } catch (err) {
...
function resizeImageToDataURL(file) {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const maxW = ESM_CONFIG.IMAGE_MAX_WIDTH;
      const maxH = ESM_CONFIG.IMAGE_MAX_HEIGHT;
      ...
      resolve(canvas.toDataURL('image/jpeg', ESM_CONFIG.IMAGE_JPEG_QUALITY));
    };
