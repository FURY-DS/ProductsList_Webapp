/* =====================================================
   smartstore-state.js - 토스쇼핑 상태 및 카드 데이터 모델
   ===================================================== */

const tossshoppingState = {
  cards: []
};

/** 새 토스쇼핑 카드 객체 생성 */
function newTOSSSHOPPINGCard() {
  return {
    id: generateId(),
    isEditing: true,
    isCollapsed: false,
...
/** ID로 토스쇼핑 카드 찾기 */
function findTOSSSHOPPINGCard(id) {
  return tossshoppingState.cards.find(c => c.id === id);
}

/** ID로 토스쇼핑 카드 인덱스 찾기 */
function findTOSSSHOPPINGCardIndex(id) {
  return tossshoppingState.cards.findIndex(c => c.id === id);
}
