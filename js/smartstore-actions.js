...
function initSmartstoreActions() {
  ...
  document.getElementById('btn-bulk-cost').addEventListener('click', openBulkCostModalSmartstore);
...
const BULK_COST_FIELDS_Smartstore = [
  { key: 'warehouseFee', label: '창고택배비' },
  { key: 'marketFee',    label: '마켓택배비' }
];

let _bulkCostOverlayEl_Smartstore = null;

/** 비용 전체입력 모달 닫기 */
function closeBulkCostModalSmartstore() {
  if (_bulkCostOverlayEl_Smartstore) {
    _bulkCostOverlayEl_Smartstore.remove();
    _bulkCostOverlayEl_Smartstore = null;
  }
}

/** 비용 2개 항목 전체입력 모달 열기 */
function openBulkCostModalSmartstore() {
  if (smartstoreState.cards.length === 0) { showToast('등록된 상품이 없어요'); return; }
  closeBulkCostModalSmartstore();

  // 2개 입력 행 생성 (placeholder는 config의 필드 정의에서 재사용)
  let rowsHtml = '';
  BULK_COST_FIELDS_Smartstore.forEach(f => {
    const ph = (SMARTSTORE_CONFIG.FIELDS[f.key] && SMARTSTORE_CONFIG.FIELDS[f.key].placeholder) || '';
    rowsHtml +=
  ...
  overlay.innerHTML =
    '<div class="bulk-cost-modal">' +
      ...
      '<p class="bulk-cost-desc">모든 상품(' + smartstoreState.cards.length + '개)에 일괄 적용할 비용을 입력하세요.<br>비워둔 항목은 변경하지 않아요.</p>' +
      '<div class="bulk-cost-body">' + rowsHtml + '</div>' +
      '<div class="bulk-cost-actions">' +
        '<button type="button" class="bulk-cost-btn bulk-cost-btn-cancel" id="bulk-cost-cancel-Smartstore">취소</button>' +
        '<button type="button" class="bulk-cost-btn bulk-cost-btn-apply" id="bulk-cost-apply-Smartstore">적용</button>' +
      '</div>' +
    '</div>';
  document.body.appendChild(overlay);
  _bulkCostOverlayEl_Smartstore = overlay;

  overlay.querySelector('#bulk-cost-cancel-Smartstore').addEventListener('click', closeBulkCostModalSmartstore);
  overlay.addEventListener('click', (e) => {
    if (e.target === overlay) closeBulkCostModalSmartstore();
  });

  overlay.querySelector('#bulk-cost-apply-Smartstore').addEventListener('click', () => {
    // 입력된(비어 있지 않은) 항목만 수집
    const values = {};
    let filled = 0;
    ...
    const summary = BULK_COST_FIELDS_Smartstore
    ...
    showModal({
      title: '비용 전체 적용',
      text: '모든 상품(' + smartstoreState.cards.length + '개)에\n' + summary + '을(를) 일괄 적용할까요?',
      confirmText: '적용',
      onConfirm: () => {
        ...
        const result = saveSmartstore();
        renderSmartstore();
        reportSaveResult(result, SMARTSTORE_CONFIG.MESSAGES,
          '전체 ' + smartstoreState.cards.length + '개 상품의 비용 ' + filled + '개 항목이 변경되었어요');
      }
    });
  });
}
