...
function cleanElevenstCardsForSave(cards) {
  return cards.map(c => {
...
function loadElevenst() {
  try {
    ...
    elevenstState.cards = data.filter(c => c && typeof c === 'object').map(c => {
      const defaults = newElevenstCard();
    ...
    console.warn(ELEVENST_CONFIG.MESSAGES.LOAD_FAIL, e);
  }
}
