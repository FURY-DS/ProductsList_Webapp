...
/** 스마트스토어 카드 데이터를 localStorage에 저장 (동시에 백업도 갱신) */
function saveSmartstore() {
  ...
  try {
    ...
    if (!isStorageQuotaError(e) || !SMARTSTORE_CONFIG.IMAGE_REMOVE_ON_SAVE_FAIL) {
      showToast(SMARTSTORE_CONFIG.MESSAGES.SAVE_FAIL + (e.message || ''));
      return;
    }
  }
  ...
  try {
    ...
    showToast(SMARTSTORE_CONFIG.MESSAGES.SAVED_WITHOUT_IMAGES);
  } catch (e2) {
    showToast(SMARTSTORE_CONFIG.MESSAGES.SAVE_FAIL_QUOTA);
  }
}

/** 저장용 데이터에서 이미지 필드 제거 (localStorage 용량 절약) */
function cleanSmartstoreCardsForSave(cards) {
  return cards.map(c => {
...
function loadSmartstore() {
  try {
    ...
    smartstoreState.cards = data.filter(c => c && typeof c === 'object').map(c => {
      const defaults = newSmartstoreCard();
    ...
    console.warn(SMARTSTORE_CONFIG.MESSAGES.LOAD_FAIL, e);
  }
}
