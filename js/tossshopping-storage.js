/* =====================================================
   smartstore-storage.js - 토스쇼핑 데이터 저장 / 불러오기
   ===================================================== */

/** 토스쇼핑 카드 데이터를 localStorage에 저장 (동시에 백업도 갱신)
 *  @returns {{ ok: boolean, imagesRemoved?: boolean, msg?: string }}
 */
function saveTOSSSHOPPING() {
  // 먼저 이미지를 포함한 원본 데이터로 저장 시도
  try {
    const data = JSON.stringify(tossshoppingState.cards);
    localStorage.setItem(TOSSSHOPPING_CONFIG.STORAGE_KEY, data);
    try {
      localStorage.setItem(TOSSSHOPPING_CONFIG.STORAGE_KEY + '_backup', data);
    } catch (backupError) {
      console.warn('Backup save failed', backupError);
    }
    return { ok: true };
  } catch (e) {
    if (!isStorageQuotaError(e) || !TOSSSHOPPING_CONFIG.IMAGE_REMOVE_ON_SAVE_FAIL) {
      return { ok: false, msg: TOSSSHOPPING_CONFIG.MESSAGES.SAVE_FAIL + (e.message || '') };
    }
  }

  // 용량 초과 시 이미지 필드를 제거한 뒤 재시도
  const cleaned = cleanTOSSSHOPPINGCardsForSave(tossshoppingState.cards);
  try {
    ...
    tossshoppingState.cards = cleaned;
    return { ok: true, imagesRemoved: true };
  } catch (e2) {
    return { ok: false, msg: TOSSSHOPPING_CONFIG.MESSAGES.SAVE_FAIL_QUOTA };
  }
}

/** 저장용 데이터에서 이미지 필드 제거 (localStorage 용량 절약) */
function cleanTOSSSHOPPINGCardsForSave(cards) {
  return cards.map(c => {
...
/** 토스쇼핑 카드 데이터를 localStorage에서 불러오기 (손상 시 백업 복구) */
function loadTOSSSHOPPING() {
  try {
    let raw = localStorage.getItem(TOSSSHOPPING_CONFIG.STORAGE_KEY);
    let data = null;
    ...
    if (!Array.isArray(data)) {
      const backupRaw = localStorage.getItem(TOSSSHOPPING_CONFIG.STORAGE_KEY + '_backup');
      if (backupRaw) {
    ...
    tossshoppingState.cards = data.filter(c => c && typeof c === 'object').map(c => {
      const defaults = newTOSSSHOPPINGCard();
    ...
    console.warn(TOSSSHOPPING_CONFIG.MESSAGES.LOAD_FAIL, e);
  }
}
...
function loadProductlistDataTOSSSHOPPING() {
  try {
    const raw = localStorage.getItem(TOSSSHOPPING_CONFIG.PRODUCTLIST_STORAGE_KEY);
    if (!raw) return [];
