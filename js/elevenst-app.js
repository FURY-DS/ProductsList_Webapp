...
async function initElevenst() {
  ...
  await clearStalePageDataIfServerEmpty([ELEVENST_CONFIG.STORAGE_KEY, ELEVENST_CONFIG.PRODUCTLIST_STORAGE_KEY], ELEVENST_CONFIG.STORAGE_KEY);

  loadElevenst();

  // === cloud-sync-init ===
  CloudSync.init(ELEVENST_CONFIG.STORAGE_KEY);
  await cloudPullAndRenderPage(ELEVENST_CONFIG, elevenstState || state, newElevenstCard, renderElevenst);
  startPageAutoSync(ELEVENST_CONFIG, elevenstState || state, newElevenstCard, renderElevenst, 10000);
...
function bindElevenstPageLifecycle() {
  ...
  if (btnCloudSync) {
    btnCloudSync.addEventListener('click', () => manualCloudSyncPage(ELEVENST_CONFIG, elevenstState || state, newElevenstCard, renderElevenst));
  }
