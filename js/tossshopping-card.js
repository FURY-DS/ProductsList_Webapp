/* =====================================================
   tossshopping-card.js - 토스쇼핑 카드 렌더링 및 보드 관리
   ===================================================== */

let tossshoppingBoardEl = null;

/** 보드 요소 초기화 */
function initTOSSSHOPPINGBoard() {
  tossshoppingBoardEl = document.getElementById('board');
  document.documentElement.style.setProperty('--columns', TOSSSHOPPING_CONFIG.COLUMNS);
}

/** 상품리스트에서 판매자상품코드로 상품 찾기 (대소문자 구분 없이 정확 일치) */
function lookupProductlistByCodeTOSSSHOPPING(code) {
  return lookupProductlistByCodeTOSSSHOPPINGFrom(code, loadProductlistDataTOSSSHOPPING());
}

/** 상품리스트에서 코드에 해당하는 상품 이미지 가져오기 */
function getProductlistImageTOSSSHOPPING(code) {
  const product = lookupProductlistByCodeTOSSSHOPPING(code);
  return product && product.image ? product.image : '';
}

/** 토스쇼핑 카드용 이미지 결정 (직접 업로드한 사진 우선, 없으면 상품리스트 조회) */
function getTOSSSHOPPINGImage(card) {
  if (card.image) return card.image;
  return getProductlistImageTOSSSHOPPING(card.sellerCode);
}

/** 복수품 항목용 이미지 결정 (직접 업로드한 사진 우선, 없으면 상품리스트 조회) */
function getBundleItemImageTOSSSHOPPING(item) {
  if (item.image) return item.image;
  return getProductlistImageTOSSSHOPPING(item.sellerCode);
}

/** 상품리스트 상품의 총합 계산 */
function computeProductlistTotalTOSSSHOPPING(product) {
...
/** 모든 카드 한 번에 접기/펼치기 */
function toggleAllTOSSSHOPPINGCards(collapse) {
  if (tossshoppingState.cards.length === 0) return;
  tossshoppingState.cards.forEach(c => c.isCollapsed = collapse);
  const result = saveTOSSSHOPPING();
  renderTOSSSHOPPING();
  reportSaveResult(result, TOSSSHOPPING_CONFIG.MESSAGES, collapse ? TOSSSHOPPING_CONFIG.MESSAGES.ALL_COLLAPSED : TOSSSHOPPING_CONFIG.MESSAGES.ALL_EXPANDED);
}

/** 전체 렌더링 */
function renderTOSSSHOPPING() {
  tossshoppingBoardEl.innerHTML = '';
  updateTOSSSHOPPINGSearchCount();
  updateTOSSSHOPPINGToggleAllButton();

  if (tossshoppingState.cards.length === 0) {
    renderTOSSSHOPPINGEmptyState(TOSSSHOPPING_CONFIG.MESSAGES.EMPTY_TITLE, TOSSSHOPPING_CONFIG.MESSAGES.EMPTY_DESC, true);
    return;
  }

  const q = tossshoppingSearchQuery.trim();
  const filtered = tossshoppingState.cards
    .map((card, originalIdx) => ({ card, originalIdx }))
    .filter(({ card }) => matchesTOSSSHOPPINGQuery(card, q));

  if (filtered.length === 0) {
    renderTOSSSHOPPINGEmptyState(TOSSSHOPPING_CONFIG.MESSAGES.NO_RESULT_TITLE, TOSSSHOPPING_CONFIG.MESSAGES.NO_RESULT_DESC(escapeAttr(q)), false);
    return;
  }

  const cols = [];
  for (let i = 0; i < TOSSSHOPPING_CONFIG.COLUMNS; i++) {
    cols.push(document.createElement('div'));
  }
  filtered.forEach((entry, idx) => {
    cols[idx % TOSSSHOPPING_CONFIG.COLUMNS].appendChild(renderTOSSSHOPPINGCard(entry.card, entry.originalIdx));
  });
  cols.forEach(c => tossshoppingBoardEl.appendChild(c));
}

/** 빈 상태 렌더링 */
function renderTOSSSHOPPINGEmptyState(title, desc, showAddBtn) {
  const empty = document.createElement('div');
  empty.className = 'empty';
  empty.style.gridColumn = '1 / -1';
  let html = `<h2>${title}</h2><p>${desc}</p>`;
  if (showAddBtn) {
    html += `<button class="btn btn-add" id="empty-add">${TOSSSHOPPING_CONFIG.MESSAGES.EMPTY_ADD_BTN}</button>`;
  }
  empty.innerHTML = html;
  tossshoppingBoardEl.appendChild(empty);
  if (showAddBtn) {
    document.getElementById('empty-add').addEventListener('click', addTOSSSHOPPINGCard);
  }
}

/** 개별 카드 렌더링 */
function renderTOSSSHOPPINGCard(card, idx) {
  const wrap = document.createElement('div');
  wrap.className = 'card tossshopping-card smartstore-card'
  ...
  const actionBtn = card.isEditing
    ? `<button class="btn btn-add" data-action="save" title="입력한 내용 저장">${TOSSSHOPPING_CONFIG.MESSAGES.BTN_SAVE}</button>`
    : `<button class="btn btn-cancel" data-action="edit" title="다시 수정하기">${TOSSSHOPPING_CONFIG.MESSAGES.BTN_EDIT}</button>`;

  const toggleBtn = card.isCollapsed
    ? `<button class="btn btn-toggle" data-action="expand" title="상세 정보 펼치기">${TOSSSHOPPING_CONFIG.MESSAGES.BTN_EXPAND}</button>`
    : `<button class="btn btn-toggle" data-action="collapse" title="상세 정보 숨기기">${TOSSSHOPPING_CONFIG.MESSAGES.BTN_COLLAPSE}</button>`;

  const sellerCodeHtml = !card.isBundle && card.isEditing
    ? `<div class="field seller-code-field"><label>${TOSSSHOPPING_CONFIG.FIELDS.sellerCode.label}</label><input type="text" name="sellerCode" value="${escapeAttr(card.sellerCode)}" placeholder="${escapeAttr(TOSSSHOPPING_CONFIG.FIELDS.sellerCode.placeholder)}" /></div>`
    : (!card.isBundle && !card.isEditing
        ? `<span class="card-badge" title="${escapeAttr(TOSSSHOPPING_CONFIG.FIELDS.sellerCode.label)}">${escapeAttr(card.sellerCode || '')}</span>`
        : `<span class="card-badge bundle-badge">${escapeAttr(TOSSSHOPPING_CONFIG.MESSAGES.BUNDLE)}</span>`);
  ...
  // ----- 본문 -----
  if (card.isCollapsed) {
    wrap.appendChild(renderTOSSSHOPPINGPhotoRow(card));
  } else {
    wrap.appendChild(renderTOSSSHOPPINGPhotoRow(card));
    wrap.appendChild(renderTOSSSHOPPINGCalcRows(card));
    if (card.isBundle) {
    ...
    if (!card.isBundle && card.isEditing) {
      ...
      bundleAdd.innerHTML = `<button class="btn btn-toggle" data-action="add-bundle-mode">${TOSSSHOPPING_CONFIG.MESSAGES.BTN_ADD_BUNDLE}</button>`;
    ...
    footer.innerHTML = `
      <button class="btn btn-add" data-action="add">${TOSSSHOPPING_CONFIG.MESSAGES.BTN_ADD}</button>
      <button class="btn btn-remove" data-action="delete">${TOSSSHOPPING_CONFIG.MESSAGES.BTN_DELETE}</button>
    `;
    wrap.appendChild(footer);
  }

  bindTOSSSHOPPINGCardEvents(wrap, card);
  return wrap;
}

/** 사진 + 상품명/옵션명 row */
function renderTOSSSHOPPINGPhotoRow(card) {
  const row = document.createElement('div');
  row.className = 'card-row';

  const imgSrc = getTOSSSHOPPINGImage(card);
  ...
  fields.appendChild(makeTOSSSHOPPINGField('name', card.name, !card.isEditing));
  fields.appendChild(makeTOSSSHOPPINGField('option', card.option, !card.isEditing));
...
function renderTOSSSHOPPINGCalcRows(card) {
  ...
  row1.appendChild(makeTOSSSHOPPINGField('finalCost', card.finalCost, true));
  row1.appendChild(makeTOSSSHOPPINGField('sellingPrice', card.sellingPrice, !card.isEditing));
  row1.appendChild(makeTOSSSHOPPINGFeeField(card));
  container.appendChild(row1);

  // 2행: 창고택배비 | 마켓택배비 | 최종이익(반) | 2개이상구매(반)
  const row2 = document.createElement('div');
  row2.className = 'field-row split-profit';
  row2.appendChild(makeTOSSSHOPPINGField('warehouseFee', card.warehouseFee, !card.isEditing));
  row2.appendChild(makeTOSSSHOPPINGField('marketFee', card.marketFee, !card.isEditing));
  row2.appendChild(makeTOSSSHOPPINGField('finalProfit', card.finalProfit, true));
  row2.appendChild(makeTOSSSHOPPINGField('multiBuyProfit', card.multiBuyProfit, true));
...
function makeTOSSSHOPPINGFeeField(card) {
  const f = document.createElement('div');
  f.className = 'field fee-field';
  const def = TOSSSHOPPING_CONFIG.FIELDS.feeRate;
  ...
  f.innerHTML = `
    ...
    <input type="text" name="feeAmount" class="highlight" readonly value="${escapeAttr(formatNumber(parseNum(card.feeAmount)))}" placeholder="${escapeAttr(TOSSSHOPPING_CONFIG.FIELDS.feeAmount.placeholder)}" />
  `;
  return f;
}

/** 단일 입력 필드 생성 */
function makeTOSSSHOPPINGField(name, value, readonly) {
  const def = TOSSSHOPPING_CONFIG.FIELDS[name];
...
function renderBundleSection(card) {
  ...
  title.textContent = TOSSSHOPPING_CONFIG.MESSAGES.BUNDLE;
  ...
  if (card.isEditing) {
    ...
    addBtn.textContent = TOSSSHOPPING_CONFIG.MESSAGES.BTN_ADD_ITEM + ' ' + TOSSSHOPPING_CONFIG.MESSAGES.BUNDLE;
...
function renderBundleItem(card, item, idx) {
  ...
  const thumbSrc = getBundleItemImageTOSSSHOPPING(item);
  ...
  const roAttr = card.isEditing ? '' : 'readonly';
  codeField.innerHTML = `
    <label>${TOSSSHOPPING_CONFIG.FIELDS.sellerCode.label}</label>
    <input type="text" name="itemSellerCode" value="${escapeAttr(item.sellerCode)}" placeholder="${escapeAttr(TOSSSHOPPING_CONFIG.FIELDS.sellerCode.placeholder)}" ${roAttr} />
  `;
  ...
  totalField.innerHTML = `
    <label>${TOSSSHOPPING_CONFIG.FIELDS.itemTotal.label}</label>
    <input type="text" name="itemTotal" class="highlight" readonly value="${escapeAttr(formatNumber(parseNum(item.total)))}" placeholder="${escapeAttr(TOSSSHOPPING_CONFIG.FIELDS.itemTotal.placeholder)}" />
  `;
  ...
  if (card.isEditing) {
    ...
    removeBtn.textContent = TOSSSHOPPING_CONFIG.MESSAGES.BTN_REMOVE_ITEM;
...
/** 카드 내 이벤트 바인딩 */
function bindTOSSSHOPPINGCardEvents(wrap, card) {
  // 입력 변경 (실시간 계산용)
  wrap.addEventListener('input', (e) => {
    const t = e.target;
    if (!t.name) return;
    const c = findTOSSSHOPPINGCard(card.id);
    if (!c) return;
    ...
    if (c.isEditing) {
      ...
      updateTOSSSHOPPINGCalcDisplay(wrap, c);
    }
  });
  ...
  wrap.addEventListener('change', (e) => {
    ...
    if (t.name === 'sellerCode') {
      ...
      updateSingleProductFromCodeTOSSSHOPPING(c, t.value);
      reportSaveResult(saveTOSSSHOPPING(), TOSSSHOPPING_CONFIG.MESSAGES);
      renderTOSSSHOPPING();
      return;
    }
    ...
    if (t.name === 'itemSellerCode') {
      ...
      if (itemId) {
        updateBundleItemFromCodeTOSSSHOPPING(c, itemId, t.value);
        reportSaveResult(saveTOSSSHOPPING(), TOSSSHOPPING_CONFIG.MESSAGES);
        renderTOSSSHOPPING();
      }
    }
  ...
  wrap.addEventListener('click', (e) => {
    ...
    if (photoBox || bundleThumb) {
      ...
      if (photoBox) {
        triggerTOSSSHOPPINGPhotoUpload(c.id, 'card');
      } else {
        const itemId = bundleThumb.closest('.bundle-item')?.dataset.itemId;
        if (itemId) triggerTOSSSHOPPINGPhotoUpload(c.id, 'bundle', itemId);
      }
      return;
    }
    ...
    if (action === 'add') {
      addTOSSSHOPPINGCardAfter(card.id);
    } else if (action === 'delete') {
      confirmDeleteTOSSSHOPPING(card.id);
    } else if (action === 'save') {
      saveTOSSSHOPPINGCard(card.id);
    } else if (action === 'edit') {
      editTOSSSHOPPINGCard(card.id);
    } else if (action === 'collapse' || action === 'expand') {
      toggleTOSSSHOPPINGCollapse(card.id);
...
function resolveTOSSSHOPPINGCards() {
  const products = loadProductlistDataTOSSSHOPPING();
  ...
  tossshoppingState.cards.forEach(c => {
    if (c.isBundle) {
      c.bundleItems.forEach(item => {
        ...
        const product = lookupProductlistByCodeTOSSSHOPPINGFrom(item.sellerCode, products);
        if (!product) return; // 매칭 실패 시 기존 데이터 유지
        if (product.name) item.name = product.name;
        if (product.option) item.option = product.option;
        item.total = computeProductlistTotalTOSSSHOPPING(product);
      });
    } else if (c.sellerCode && c.sellerCode.trim()) {
      const product = lookupProductlistByCodeTOSSSHOPPINGFrom(c.sellerCode, products);
      if (!product) return; // 매칭 실패 시 기존 데이터 유지
      if (product.name) c.name = product.name;
      if (product.option) c.option = product.option;
      c.finalCost = computeProductlistTotalTOSSSHOPPING(product);
    }
    recalcTOSSSHOPPINGCard(c);
  });
}
...
function lookupProductlistByCodeTOSSSHOPPINGFrom(code, products) {
...
function updateSingleProductFromCodeTOSSSHOPPING(card, code) {
  card.sellerCode = code;
  if (!code || !code.trim()) return;
  const product = lookupProductlistByCodeTOSSSHOPPING(code);
  if (!product) {
    showToast(TOSSSHOPPING_CONFIG.MESSAGES.PRODUCT_NOT_FOUND(code));
    return;
  }
  // 이미지는 상품리스트에서 실시간 조회하므로 카드에 저장하지 않음
  card.name = product.name || '';
  card.option = product.option || '';
  card.finalCost = computeProductlistTotalTOSSSHOPPING(product);
  recalcTOSSSHOPPINGCard(card);
}

/** 복수품 항목: 판매자상품코드로 상품리스트에서 정보 불러오기 */
function updateBundleItemFromCodeTOSSSHOPPING(card, itemId, code) {
...
/** 카드 저장 */
function saveTOSSSHOPPINGCard(cardId) {
  const c = findTOSSSHOPPINGCard(cardId);
  if (!c) return;
  syncTOSSSHOPPINGCardFromDOM(cardId);
  c.isEditing = false;
  const result = saveTOSSSHOPPING();
  renderTOSSSHOPPING();
  reportSaveResult(result, TOSSSHOPPING_CONFIG.MESSAGES, TOSSSHOPPING_CONFIG.MESSAGES.SAVED);
}

/** 렌더링된 DOM의 현재 입력값을 상태 객체에 동기화 (저장 직전 안전 장치) */
function syncTOSSSHOPPINGCardFromDOM(cardId) {
...
/** 카드 수정 모드 진입 */
function editTOSSSHOPPINGCard(cardId) {
  const c = findTOSSSHOPPINGCard(cardId);
  if (!c) return;
  c.isEditing = true;
  c.isCollapsed = false;
...
/** 카드 접기/펼치기 */
function toggleTOSSSHOPPINGCollapse(cardId) {
  const c = findTOSSSHOPPINGCard(cardId);
  if (!c) return;
...
function triggerTOSSSHOPPINGPhotoUpload(cardId, type, itemId) {
  ...
  input.addEventListener('change', async (e) => {
    ...
    if (file.size > TOSSSHOPPING_CONFIG.IMAGE_MAX_SIZE_BYTES) {
    ...
    try {
      ...
      const result = saveTOSSSHOPPING();
      renderTOSSSHOPPING();
      reportSaveResult(result, TOSSSHOPPING_CONFIG.MESSAGES, '사진을 변경했어요');
    } catch (err) {
...
function resizeImageToDataURL(file) {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const maxW = TOSSSHOPPING_CONFIG.IMAGE_MAX_WIDTH;
      const maxH = TOSSSHOPPING_CONFIG.IMAGE_MAX_HEIGHT;
      ...
      resolve(canvas.toDataURL('image/jpeg', TOSSSHOPPING_CONFIG.IMAGE_JPEG_QUALITY));
    };
