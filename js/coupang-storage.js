...
/** 쿠팡 카드 데이터를 localStorage에 저장 (동시에 백업도 갱신) */
function saveCoupang() {
  // 먼저 이미지를 포함한 원본 데이터로 저장 시도
  try {
    const data = JSON.stringify(coupangState.cards);
    localStorage.setItem(COUPANG_CONFIG.STORAGE_KEY, data);
    localStorage.setItem(COUPANG_CONFIG.STORAGE_KEY + '_backup', data);
    return;
  } catch (e) {
    if (!isStorageQuotaError(e) || !COUPANG_CONFIG.IMAGE_REMOVE_ON_SAVE_FAIL) {
      showToast(COUPANG_CONFIG.MESSAGES.SAVE_FAIL + (e.message || ''));
      return;
    }
  }

  // 용량 초과 시 이미지 필드를 제거한 뒤 재시도
  const cleaned = cleanCoupangCardsForSave(coupangState.cards);
  try {
    const data = JSON.stringify(cleaned);
    localStorage.setItem(COUPANG_CONFIG.STORAGE_KEY, data);
    localStorage.setItem(COUPANG_CONFIG.STORAGE_KEY + '_backup', data);
    // 메모리 상태도 이미지 없이 동기화 (다음 저장 시도 방지)
    coupangState.cards = cleaned;
    showToast(COUPANG_CONFIG.MESSAGES.SAVED_WITHOUT_IMAGES);
  } catch (e2) {
    showToast(COUPANG_CONFIG.MESSAGES.SAVE_FAIL_QUOTA);
  }
}

/** 저장용 데이터에서 이미지 필드 제거 (localStorage 용량 절약) */
function cleanCoupangCardsForSave(cards) {
  return cards.map(c => {
...
/** 쿠팡 카드 데이터를 localStorage에서 불러오기 (손상 시 백업 복구) */
function loadCoupang() {
  try {
    let raw = localStorage.getItem(COUPANG_CONFIG.STORAGE_KEY);
    let data = null;
    ...
    coupangState.cards = data.filter(c => c && typeof c === 'object').map(c => {
      const defaults = newCoupangCard();
    ...
    console.warn(COUPANG_CONFIG.MESSAGES.LOAD_FAIL, e);
  }
}
...
function loadProductlistData() {
  try {
    const raw = localStorage.getItem(COUPANG_CONFIG.PRODUCTLIST_STORAGE_KEY);
    if (!raw) return [];
