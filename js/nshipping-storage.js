...
function saveNshipping() {
  const storageKey = getUserScopedKey(NSHIPPING_CONFIG.STORAGE_KEY);
...
function loadNshipping() {
  try {
    const storageKey = migrateLegacyKeyToUserScope(NSHIPPING_CONFIG.STORAGE_KEY, Array.isArray);
...
function loadProductlistDataForNshipping() {
  try {
    const raw = getUserScopedItemWithFallback(NSHIPPING_CONFIG.PRODUCTLIST_STORAGE_KEY);
    if (!raw) return [];
