 ...
 */
function saveALWAYS() {
  const storageKey = getUserScopedKey(ALWAYS_CONFIG.STORAGE_KEY);
...
function loadALWAYS() {
  try {
    const storageKey = migrateLegacyKeyToUserScope(ALWAYS_CONFIG.STORAGE_KEY, Array.isArray);
...
function loadProductlistDataALWAYS() {
  try {
    const raw = getUserScopedItemWithFallback(ALWAYS_CONFIG.PRODUCTLIST_STORAGE_KEY);
    if (!raw) return [];
