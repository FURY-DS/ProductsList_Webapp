/* =====================================================
   always-card.js - 올웨이즈 카드 렌더링 및 보드 관리
   ===================================================== */

let alwaysBoardEl = null;

/** 보드 요소 초기화 */
function initALWAYSBoard() {
  alwaysBoardEl = document.getElementById('board');
  document.documentElement.style.setProperty('--columns', ALWAYS_CONFIG.COLUMNS);
}

/** 상품리스트에서 판매자상품코드로 상품 찾기 (대소문자 구분 없이 정확 일치) */
function lookupProductlistByCodeALWAYS(code) {
  return lookupProductlistByCodeALWAYSFrom(code, loadProductlistDataALWAYS());
}

/** 상품리스트에서 코드에 해당하는 상품 이미지 가져오기 */
function getProductlistImageALWAYS(code) {
  const product = lookupProductlistByCodeALWAYS(code);
  return product && product.image ? product.image : '';
}

/** 올웨이즈 카드용 이미지 결정 (직접 업로드한 사진 우선, 없으면 상품리스트 조회) */
function getALWAYSImage(card) {
  if (card.image) return card.image;
  return getProductlistImageALWAYS(card.sellerCode);
}

/** 복수품 항목용 이미지 결정 (직접 업로드한 사진 우선, 없으면 상품리스트 조회) */
function getBundleItemImageALWAYS(item) {
  if (item.image) return item.image;
  return getProductlistImageALWAYS(item.sellerCode);
}

/** 상품리스트 상품의 총합 계산 */
function computeProductlistTotalALWAYS(product) {
...
/** 모든 카드 한 번에 접기/펼치기 */
function toggleAllALWAYSCards(collapse) {
  if (alwaysState.cards.length === 0) return;
  alwaysState.cards.forEach(c => c.isCollapsed = collapse);
  const result = saveALWAYS();
  renderALWAYS();
  reportSaveResult(result, ALWAYS_CONFIG.MESSAGES, collapse ? ALWAYS_CONFIG.MESSAGES.ALL_COLLAPSED : ALWAYS_CONFIG.MESSAGES.ALL_EXPANDED);
}

/** 전체 렌더링 */
function renderALWAYS() {
  alwaysBoardEl.innerHTML = '';
  updateALWAYSSearchCount();
  updateALWAYSToggleAllButton();

  if (alwaysState.cards.length === 0) {
    renderALWAYSEmptyState(ALWAYS_CONFIG.MESSAGES.EMPTY_TITLE, ALWAYS_CONFIG.MESSAGES.EMPTY_DESC, true);
    return;
  }

  const q = alwaysSearchQuery.trim();
  const filtered = alwaysState.cards
    .map((card, originalIdx) => ({ card, originalIdx }))
    .filter(({ card }) => matchesALWAYSQuery(card, q));

  if (filtered.length === 0) {
    renderALWAYSEmptyState(ALWAYS_CONFIG.MESSAGES.NO_RESULT_TITLE, ALWAYS_CONFIG.MESSAGES.NO_RESULT_DESC(escapeAttr(q)), false);
    return;
  }

  const cols = [];
  for (let i = 0; i < ALWAYS_CONFIG.COLUMNS; i++) {
    cols.push(document.createElement('div'));
  }
  filtered.forEach((entry, idx) => {
    cols[idx % ALWAYS_CONFIG.COLUMNS].appendChild(renderALWAYSCard(entry.card, entry.originalIdx));
  });
  cols.forEach(c => alwaysBoardEl.appendChild(c));
}

/** 빈 상태 렌더링 */
function renderALWAYSEmptyState(title, desc, showAddBtn) {
  const empty = document.createElement('div');
  empty.className = 'empty';
  empty.style.gridColumn = '1 / -1';
  let html = `<h2>${title}</h2><p>${desc}</p>`;
  if (showAddBtn) {
    html += `<button class="btn btn-add" id="empty-add">${ALWAYS_CONFIG.MESSAGES.EMPTY_ADD_BTN}</button>`;
  }
  empty.innerHTML = html;
  alwaysBoardEl.appendChild(empty);
  if (showAddBtn) {
    document.getElementById('empty-add').addEventListener('click', addALWAYSCard);
  }
}

/** 개별 카드 렌더링 */
function renderALWAYSCard(card, idx) {
  const wrap = document.createElement('div');
  wrap.className = 'card always-card smartstore-card'
  ...
  const actionBtn = card.isEditing
    ? `<button class="btn btn-add" data-action="save" title="입력한 내용 저장">${ALWAYS_CONFIG.MESSAGES.BTN_SAVE}</button>`
    : `<button class="btn btn-cancel" data-action="edit" title="다시 수정하기">${ALWAYS_CONFIG.MESSAGES.BTN_EDIT}</button>`;

  const toggleBtn = card.isCollapsed
    ? `<button class="btn btn-toggle" data-action="expand" title="상세 정보 펼치기">${ALWAYS_CONFIG.MESSAGES.BTN_EXPAND}</button>`
    : `<button class="btn btn-toggle" data-action="collapse" title="상세 정보 숨기기">${ALWAYS_CONFIG.MESSAGES.BTN_COLLAPSE}</button>`;

  const sellerCodeHtml = !card.isBundle && card.isEditing
    ? `<div class="field seller-code-field"><label>${ALWAYS_CONFIG.FIELDS.sellerCode.label}</label><input type="text" name="sellerCode" value="${escapeAttr(card.sellerCode)}" placeholder="${escapeAttr(ALWAYS_CONFIG.FIELDS.sellerCode.placeholder)}" /></div>`
    : (!card.isBundle && !card.isEditing
        ? `<span class="card-badge" title="${escapeAttr(ALWAYS_CONFIG.FIELDS.sellerCode.label)}">${escapeAttr(card.sellerCode || '')}</span>`
        : `<span class="card-badge bundle-badge">${escapeAttr(ALWAYS_CONFIG.MESSAGES.BUNDLE)}</span>`);
  ...
  // ----- 본문 -----
  if (card.isCollapsed) {
    wrap.appendChild(renderALWAYSPhotoRow(card));
  } else {
    wrap.appendChild(renderALWAYSPhotoRow(card));
    wrap.appendChild(renderALWAYSCalcRows(card));
    if (card.isBundle) {
    ...
    if (!card.isBundle && card.isEditing) {
      ...
      bundleAdd.innerHTML = `<button class="btn btn-toggle" data-action="add-bundle-mode">${ALWAYS_CONFIG.MESSAGES.BTN_ADD_BUNDLE}</button>`;
    ...
    footer.innerHTML = `
      <button class="btn btn-add" data-action="add">${ALWAYS_CONFIG.MESSAGES.BTN_ADD}</button>
      <button class="btn btn-remove" data-action="delete">${ALWAYS_CONFIG.MESSAGES.BTN_DELETE}</button>
    `;
    wrap.appendChild(footer);
  }

  bindALWAYSCardEvents(wrap, card);
  return wrap;
}

/** 사진 + 상품명/옵션명 row */
function renderALWAYSPhotoRow(card) {
  const row = document.createElement('div');
  row.className = 'card-row';

  const imgSrc = getALWAYSImage(card);
  ...
  fields.appendChild(makeALWAYSField('name', card.name, !card.isEditing));
  fields.appendChild(makeALWAYSField('option', card.option, !card.isEditing));
...
function renderALWAYSCalcRows(card) {
  ...
  row1.appendChild(makeALWAYSField('finalCost', card.finalCost, true));
  row1.appendChild(makeALWAYSField('sellingPrice', card.sellingPrice, !card.isEditing));
  row1.appendChild(makeALWAYSFeeField(card));
  container.appendChild(row1);

  // 2행: 창고택배비 | 마켓택배비 | 최종이익(반) | 2개이상구매(반)
  const row2 = document.createElement('div');
  row2.className = 'field-row split-profit';
  row2.appendChild(makeALWAYSField('warehouseFee', card.warehouseFee, !card.isEditing));
  row2.appendChild(makeALWAYSField('marketFee', card.marketFee, !card.isEditing));
  row2.appendChild(makeALWAYSField('finalProfit', card.finalProfit, true));
  row2.appendChild(makeALWAYSField('multiBuyProfit', card.multiBuyProfit, true));
...
function makeALWAYSFeeField(card) {
  const f = document.createElement('div');
  f.className = 'field fee-field';
  const def = ALWAYS_CONFIG.FIELDS.feeRate;
  ...
  f.innerHTML = `
    ...
    <input type="text" name="feeAmount" class="highlight" readonly value="${escapeAttr(formatNumber(parseNum(card.feeAmount)))}" placeholder="${escapeAttr(ALWAYS_CONFIG.FIELDS.feeAmount.placeholder)}" />
  `;
  return f;
}

/** 단일 입력 필드 생성 */
function makeALWAYSField(name, value, readonly) {
  const def = ALWAYS_CONFIG.FIELDS[name];
...
function renderBundleSection(card) {
  ...
  title.textContent = ALWAYS_CONFIG.MESSAGES.BUNDLE;
  ...
  if (card.isEditing) {
    ...
    addBtn.textContent = ALWAYS_CONFIG.MESSAGES.BTN_ADD_ITEM + ' ' + ALWAYS_CONFIG.MESSAGES.BUNDLE;
...
function renderBundleItem(card, item, idx) {
  ...
  const thumbSrc = getBundleItemImageALWAYS(item);
  ...
  const roAttr = card.isEditing ? '' : 'readonly';
  codeField.innerHTML = `
    <label>${ALWAYS_CONFIG.FIELDS.sellerCode.label}</label>
    <input type="text" name="itemSellerCode" value="${escapeAttr(item.sellerCode)}" placeholder="${escapeAttr(ALWAYS_CONFIG.FIELDS.sellerCode.placeholder)}" ${roAttr} />
  `;
  ...
  totalField.innerHTML = `
    <label>${ALWAYS_CONFIG.FIELDS.itemTotal.label}</label>
    <input type="text" name="itemTotal" class="highlight" readonly value="${escapeAttr(formatNumber(parseNum(item.total)))}" placeholder="${escapeAttr(ALWAYS_CONFIG.FIELDS.itemTotal.placeholder)}" />
  `;
  ...
  if (card.isEditing) {
    ...
    removeBtn.textContent = ALWAYS_CONFIG.MESSAGES.BTN_REMOVE_ITEM;
...
/** 카드 내 이벤트 바인딩 */
function bindALWAYSCardEvents(wrap, card) {
  // 입력 변경 (실시간 계산용)
  wrap.addEventListener('input', (e) => {
    const t = e.target;
    if (!t.name) return;
    const c = findALWAYSCard(card.id);
    if (!c) return;
    ...
    if (c.isEditing) {
      ...
      updateALWAYSCalcDisplay(wrap, c);
    }
  });
  ...
  wrap.addEventListener('change', (e) => {
    ...
    if (t.name === 'sellerCode') {
      ...
      updateSingleProductFromCodeALWAYS(c, t.value);
      reportSaveResult(saveALWAYS(), ALWAYS_CONFIG.MESSAGES);
      renderALWAYS();
      return;
    }
    ...
    if (t.name === 'itemSellerCode') {
      ...
      if (itemId) {
        updateBundleItemFromCodeALWAYS(c, itemId, t.value);
        reportSaveResult(saveALWAYS(), ALWAYS_CONFIG.MESSAGES);
        renderALWAYS();
      }
    }
  ...
  wrap.addEventListener('click', (e) => {
    ...
    if (photoBox || bundleThumb) {
      ...
      if (photoBox) {
        triggerALWAYSPhotoUpload(c.id, 'card');
      } else {
        const itemId = bundleThumb.closest('.bundle-item')?.dataset.itemId;
        if (itemId) triggerALWAYSPhotoUpload(c.id, 'bundle', itemId);
      }
      return;
    }
    ...
    if (action === 'add') {
      addALWAYSCardAfter(card.id);
    } else if (action === 'delete') {
      confirmDeleteALWAYS(card.id);
    } else if (action === 'save') {
      saveALWAYSCard(card.id);
    } else if (action === 'edit') {
      editALWAYSCard(card.id);
    } else if (action === 'collapse' || action === 'expand') {
      toggleALWAYSCollapse(card.id);
...
function resolveALWAYSCards() {
  const products = loadProductlistDataALWAYS();
  ...
  alwaysState.cards.forEach(c => {
    if (c.isBundle) {
      c.bundleItems.forEach(item => {
        ...
        const product = lookupProductlistByCodeALWAYSFrom(item.sellerCode, products);
        if (!product) return; // 매칭 실패 시 기존 데이터 유지
        if (product.name) item.name = product.name;
        if (product.option) item.option = product.option;
        item.total = computeProductlistTotalALWAYS(product);
      });
    } else if (c.sellerCode && c.sellerCode.trim()) {
      const product = lookupProductlistByCodeALWAYSFrom(c.sellerCode, products);
      if (!product) return; // 매칭 실패 시 기존 데이터 유지
      if (product.name) c.name = product.name;
      if (product.option) c.option = product.option;
      c.finalCost = computeProductlistTotalALWAYS(product);
    }
    recalcALWAYSCard(c);
  });
}
...
function lookupProductlistByCodeALWAYSFrom(code, products) {
...
function updateSingleProductFromCodeALWAYS(card, code) {
  card.sellerCode = code;
  if (!code || !code.trim()) return;
  const product = lookupProductlistByCodeALWAYS(code);
  if (!product) {
    showToast(ALWAYS_CONFIG.MESSAGES.PRODUCT_NOT_FOUND(code));
    return;
  }
  // 이미지는 상품리스트에서 실시간 조회하므로 카드에 저장하지 않음
  card.name = product.name || '';
  card.option = product.option || '';
  card.finalCost = computeProductlistTotalALWAYS(product);
  recalcALWAYSCard(card);
}

/** 복수품 항목: 판매자상품코드로 상품리스트에서 정보 불러오기 */
function updateBundleItemFromCodeALWAYS(card, itemId, code) {
...
/** 카드 저장 */
function saveALWAYSCard(cardId) {
  const c = findALWAYSCard(cardId);
  if (!c) return;
  syncALWAYSCardFromDOM(cardId);
  c.isEditing = false;
  const result = saveALWAYS();
  renderALWAYS();
  reportSaveResult(result, ALWAYS_CONFIG.MESSAGES, ALWAYS_CONFIG.MESSAGES.SAVED);
}

/** 렌더링된 DOM의 현재 입력값을 상태 객체에 동기화 (저장 직전 안전 장치) */
function syncALWAYSCardFromDOM(cardId) {
...
/** 카드 수정 모드 진입 */
function editALWAYSCard(cardId) {
  const c = findALWAYSCard(cardId);
  if (!c) return;
  c.isEditing = true;
  c.isCollapsed = false;
...
/** 카드 접기/펼치기 */
function toggleALWAYSCollapse(cardId) {
  const c = findALWAYSCard(cardId);
  if (!c) return;
...
function triggerALWAYSPhotoUpload(cardId, type, itemId) {
  ...
  input.addEventListener('change', async (e) => {
    ...
    if (file.size > ALWAYS_CONFIG.IMAGE_MAX_SIZE_BYTES) {
    ...
    try {
      ...
      const result = saveALWAYS();
      renderALWAYS();
      reportSaveResult(result, ALWAYS_CONFIG.MESSAGES, '사진을 변경했어요');
    } catch (err) {
...
function resizeImageToDataURL(file) {
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const maxW = ALWAYS_CONFIG.IMAGE_MAX_WIDTH;
      const maxH = ALWAYS_CONFIG.IMAGE_MAX_HEIGHT;
      ...
      resolve(canvas.toDataURL('image/jpeg', ALWAYS_CONFIG.IMAGE_JPEG_QUALITY));
    };
